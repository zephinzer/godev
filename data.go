...
// THE FOLLOWING FILE
//
// GENERATED BY GO:GENERATE AT 2019-03-11 09:58:19.362249217 +0800 HKT m=+0.008016971
//
...
// hash:feca15d37ee6b6dc7eaa39dbae316e40
const DataMakefile = `##
...
deps:
	@go mod vendor
## creates any static files if needed
generate:
	@go generate
...
.compile:
	@CGO_EMABLED=0 GO111MODULE=on \
		go build -a \
			...
			-o $(CURDIR)/$(BIN_PATH)/$(BIN_NAME)-${GOOS}-${GOARCH}${BIN_EXT}
	@chmod +x $(CURDIR)/$(BIN_PATH)/$(BIN_NAME)-${GOOS}-${GOARCH}${BIN_EXT}
	@sha256sum $(CURDIR)/$(BIN_PATH)/$(BIN_NAME)-${GOOS}-${GOARCH}${BIN_EXT} | cut -d " " -f 1 \
		> $(CURDIR)/$(BIN_PATH)/$(BIN_NAME)-${GOOS}-${GOARCH}${BIN_EXT}.sha256
