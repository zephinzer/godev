...
test: compile.linux
	...
	@$(CURDIR)/bin/godev--linux-amd64 --test --ignore .cache,.git,vendor,data

test.mac: compile.macos	
	@$(MAKE) log.debug MSG="running tests in watch mode for godev..."
	@$(CURDIR)/bin/godev--darwin-amd64 --test --ignore .cache,.git,vendor,data

test.win: compile.windows
	@$(MAKE) log.debug MSG="running tests in watch mode for godev..."
	@$(CURDIR)/bin/godev--windows-386 --test --ignore .cache,.git,vendor,data
